...
Produto no ar: **0.2.26**. Pulsos **11:30 venda** e **23:30 produto**. Login = código do bloco. Cada pulso planta D+8. Sem publicação de manhã, a venda falhou — **não** some o D+8.
...
| D5 | 05/09 | `/api/heartbeat` stub: last ship, 7-day, last research line — **SHIP 0.2.26** | HTTP 402 / x402 — só nota, sem implementar | Heartbeat na landing |
...
| D14 | 13/09 | **plantado 04/09 23:30:** se `GET /api/heartbeat` já devolve last_ship + last_research, a landing mostra essa linha (sem PII). Sem segundo caixa. Sem HTTP 402 | o stub do pulso moveu visita ou só JSON? | D22: se visit EN > 0 e pay_click EN = 0, o card EN continua from ~$1 — ainda sem botão USDT |
